...
public class QueryDispatcher : IQueryDispatcher<PostEntity>
{
    ...
    public void RegisterHandler<TQuery>(
        Func<TQuery, Task<List<PostEntity>>> handler) 
    ...
    {
        if (_handler.TryGetValue(
                query.GetType(), out Func<BaseQuery, Task<List<PostEntity>>> handler))
